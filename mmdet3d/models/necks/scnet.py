...
class SCNET(nn.Module):
    def __init__(self, num_bev_features, channel_adjust_cfg=None, num_blocks=3):
        super().__init__()

        if channel_adjust_cfg:
            self.channel_adjust = cnn.ConvModule(**channel_adjust_cfg)
        else:
            self.channel_adjust = None

        self.num_bev_features = num_bev_features

        self.layers = self.make_layer(SCBottleneck, num_bev_features, num_blocks = num_blocks)
